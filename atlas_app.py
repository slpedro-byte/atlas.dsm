...
FRAMEWORK_INFO = {
    "DMAIC": {
        ...
        "when": "Complex quality problems with historical data, 3–6 months available, Green/Black Belt expertise.",
    ...
    "A3": {
        ...
        "when": "Problem is already well-defined, low-to-medium maturity, 1–4 weeks available.",
    ...
    "8D": {
        ...
        "tools": "D0–D8 steps, Containment Action, Fishbone, 5 Whys, Mistake-Proofing"
    },
    "TRIZ": {
        "icon": "💡",
    ...
    "DMADV": {
        ...
        "when": "New product or process design, no existing process to improve, 4–9 months available.",
        "tools": "VOC, QFD/House of Quality, DFMEA, Design Verification"
    },
}

HYBRID_MAP = {
    ("DMAIC","A3"):  ("A3 + DMAIC", "Start with A3 for scoping. If root cause proves statistically complex, escalate to DMAIC."),
    ("A3","DMAIC"):  ("A3 + DMAIC", "Start with A3 for scoping. If root cause proves statistically complex, escalate to DMAIC."),
    ("Lean","PDCA"): ("Lean + PDCA", "Use PDCA to build CI culture through fast cycles, while deploying Lean (VSM + Kaizen) at system level."),
    ("PDCA","Lean"): ("Lean + PDCA", "Use PDCA to build CI culture through fast cycles, while deploying Lean (VSM + Kaizen) at system level."),
}
...
def show_direct(fw):
    ...
    if fw == "8D":
        st.error(f"**{FRAMEWORK_INFO[fw]['icon']} {fw}**")
    else:
        st.success(f"**{FRAMEWORK_INFO[fw]['icon']} {fw}**")
...
def show_topsis_stage(candidates):
    ...
    with col1:
        ...
        maturity = st.slider("🏢 Organisational maturity\n\n*CI skills and culture?*", 1, 5, 3, help="1=no CI · 3=some projects · 5=dedicated team")
    with col2:
        digital  = st.slider("💻 Digital maturity\n\n*IoT, dashboards, analytics?*", 1, 5, 3, help="1=paper-based · 5=advanced analytics")
        time     = st.slider("⏱️ Time available\n\n*How long to solve this?*", 1, 5, 3, help="1=days · 3=weeks · 5=6+ months")
    ...
    if st.button("▶ Run ATLAS", type="primary", use_container_width=True):
        ...
        for i, (fw, ci) in enumerate(ranked):
            with cols[i]:
                medal = "🥇" if i == 0 else "🥈"
                st.metric(
                    label=f"{medal} {FRAMEWORK_INFO[fw]['icon']} {fw}",
                    value=f"Cᵢ = {ci:.3f}",
        ...
        if gap < 0.15 and top_ci > 0.40 and second_ci > 0.40 and hybrid_key in HYBRID_MAP:
            ...
            st.warning(f"**⚠️ Hybrid Recommended: {hybrid_name}**")
            st.markdown(f"*Gap = {gap:.3f} — both frameworks are strong candidates. A hybrid may be superior.*")
            st.markdown(f"> {hybrid_desc}")
        else:
            st.success(f"**{FRAMEWORK_INFO[top_fw]['icon']} {top_fw}**  *(Cᵢ = {top_ci:.3f})*")
...
- 🗺️ **VSM** — map and diagnose waste at system level
- ⚡ **Kaizen Events** — rapid targeted improvement
...
<div style='text-align:center; padding:1.5rem 0 0.5rem 0;'>
    ...
    <p style='color:#888; font-size:1rem; margin-top:0.2rem;'>
        Adaptive Tool Leading problem-solving Analysis & Selection
    </p>
...
if q1 != "Select an answer...":
    ...
    if "No" in q1:
        q2b = st.radio(
            "**Q2b — Is there a technical contradiction?**\n\n*e.g. making it stronger makes it heavier; more accurate means more expensive*",
            ["Select an answer...",
        ...
        q2a = st.radio(
            "**Q2a — Has the problem already occurred?**\n\n*e.g. customer complaint received, batch rejected, NCR issued*",
            ["Select an answer...",
        ...
        if q2a != "Select an answer...":
            ...
            if "Yes" in q2a:
                q3a = st.radio(
                    "**Q3a — Is there urgency of containment?**\n\n*e.g. customer actively affected, production stopped, regulatory pressure*",
                    ["Select an answer...",
                ...
                q3b = st.radio(
                    ...
                    ["Select an answer...",
                     "Flow & waste — eliminate non-value-added activities, improve process flow",
                     "Quality & variability — reduce defects, variation, improve process capability"],
                    key="q3b"
                )
